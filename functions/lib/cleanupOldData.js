...
exports.cleanupOldData = functions.pubsub.schedule("every 24 hours").onRun(async (context) => {
    ...
    // Logic: Delete data older than retention period
    // e.g., cleanup tasks older than 90 days
    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - 90);
    const oldTasksQuery = db.collection("tasks").where("dueDate", "<", cutoff).where("status", "==", "completed");
